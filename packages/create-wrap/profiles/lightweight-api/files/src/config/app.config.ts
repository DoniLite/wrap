...
/**
 * Application configuration — Lightweight-API profile: no `database`,
 * `storage` or `email` sections, since this profile has no DB and isn't
 * assumed to need file storage or outbound mail. Add them back (see the
 * full-backend profile's `app.config.ts`) if the project grows into them.
 */
